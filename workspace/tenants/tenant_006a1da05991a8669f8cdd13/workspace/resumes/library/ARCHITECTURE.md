...
- `modes/_profile.md`：Ucareer 评估岗位时使用的个性化评分规则和叙事偏好。
...
1. 保存 JD 或评估报告到 `workspace/jobs/reports/`。
2. 判断最接近的方向母版。
...
4. 按 JD 调整：
   ...
   - bullet 关键词
   - 风险短板说明
5. 生成 PDF 到 `workspace/ops/exports/`。
...
| `workspace/ops/exports/*` | 最终导出文件 | 是 |
| `workspace/jobs/reports/*` | 岗位评估报告 | 否 |
...
Ucareer/
  ...
  resumes/
    ...
    06-enterprise-ai-rag-engineer.md
    targets/            # 具体 JD 定制版
  workspace/jobs/reports/              # 岗位评估
  workspace/ops/exports/ # PDF / HTML / DOCX 输出
...
2. `workspace/ops/exports/`：每个投递版导出 PDF。
...
flowchart TD
  ...
  R2 --> R3["导出 PDF / DOCX workspace/ops/exports/"]
  R3 --> A["投递 / 沟通 / 面试"]
...
| 模块 | 作用 | 产物 |
|---|---|---|
| 市场扫描 | 从 Boss、脉脉、猎头、官网收集 JD | `workspace/jobs/reports/*.md`、`research/*.md` |
| 需求聚类 | 找反复出现的关键词和能力要求 | `resumes/direction-clues.json` |
| 匹配引擎 | 判断现有经历能不能支撑这个岗位 | 岗位评分、缺口清单 |
| 人类输入 | 补充真实项目材料和证明 | `workspace/jobs/project-notes/*.md` |
