# Ucareer Technical Architecture Design

Date: 2026-06-05

## 1. Product Purpose

Ucareer is an AI job-search operating system and multi-agent work platform.
...
7. Incremental migration: current Ucareer files remain usable while the new app is introduced.
...
- Storage: local workspace files, `data/`, `resumes/`, `workspace/jobs/reports/`, `workspace/ops/exports/`
...
- `apps/daemon/src/resume-export-store.ts` exports resume Markdown, HTML, PDF and DOCX into `workspace/ops/exports/exports/`.
...
- List evaluation reports from `workspace/jobs/reports/`.
...
- Keep existing Ucareer files as the storage source.
...
- Keep migrated service logic in `apps/daemon/src/*-store.ts`.
...
1. Retired website workflows have equivalent React screens where still needed.
...
The first architecture slice has removed the retired website surface and uses the migrated web/daemon surface:

- `apps/daemon`: local execution boundary and TypeScript Fastify API.
- `apps/daemon/src/server.ts`: TypeScript Fastify API entry for platform capabilities such as provider inspection.
- `.ucareer/daemon.sqlite`: local SQLite fact source for daemon tasks, events, approval requests and approval decisions.
- `sync_events`: local SQLite outbox for future cloud sync; daemon exposes outbox read and mark-pushed APIs.
- `apps/web`: React/Vite/TypeScript web surface with migrated product screens.
...
- Applications migration has started: `apps/daemon/src/application-store.ts` parses `workspace/ops/data/applications.md` and `apps/web` renders application metrics and tracker rows.
- Experience assets migration has started: `apps/daemon/src/experience-store.ts` reads `workspace/jobs/project-notes` and `data/experience-metadata.json`, and `apps/web` renders experience cards.
...
- Evidence fulfillment writes have started: `apps/daemon/src/evidence-store.ts` validates request IDs and target paths, then appends submitted evidence to safe `workspace/jobs/project-notes/*` targets from the web console.
...
- `docs/archive/architecture-tasks-2026-06-05.md`: workstream split for web, daemon, shared contracts, provider adapters, data migration and cloud sync. Mobile is documented as deferred.
