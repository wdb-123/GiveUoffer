# Ucareer Architecture Task Split

Date: 2026-06-05

This document turns the target architecture into concrete workstreams. The implementation now uses `apps/web`, `apps/daemon` and `packages/*` as the active product surface.

## Workstreams

| Workstream | Owner role | Scope | First deliverable |
| --- | --- | --- | --- |
| Web console | Frontend agent | React/Vite screens and typed API client | `apps/web` product screens |
| Local daemon | Backend agent | Local HTTP/WebSocket server, migrated stores, health checks | `apps/daemon` TypeScript Fastify API |
...
1. Keep `apps/daemon` as the local execution boundary.
2. Keep `packages/shared` and `packages/agent-core` as the contract layer.
3. Keep `apps/web` as the React/Vite surface.
4. Continue migrating remaining file workflows into daemon stores.
5. Keep provider adapters behind the daemon contract.
...
| Area | Status | Notes |
| --- | --- | --- |
| Retired website | Removed | old static website display code has been deleted |
| Daemon API | Started | `npm run daemon` starts a Fastify TS API with `/health` and provider endpoints |
