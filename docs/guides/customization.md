...
## Portals (`workspace/profile/portals.yml`)

Copy from `workspace/ops/templates/portals.example.yml` and customize:
...
## CV Template (`workspace/ops/templates/cv-template.html`)

The HTML template uses these design tokens:
- **Fonts**: Space Grotesk (headings) + DM Sans (body) -- self-hosted in `workspace/ops/templates/fonts/`
- **Colors**: Cyan primary (`hsl(187,74%,32%)`) + Purple accent (`hsl(270,70%,45%)`)
- **Layout**: Single-column, ATS-optimized

To customize fonts/colors, edit the CSS in the template. Update font files in `workspace/ops/templates/fonts/` if switching fonts.
...
Ucareer can integrate with external systems via agent hooks. Example hooks:
...
Hook configuration depends on the agent runner being used.

## States (`workspace/ops/templates/states.yml`)

The canonical states rarely need changing. If you add new states, update:
1. `workspace/ops/templates/states.yml`
2. `scripts/cli/normalize-statuses.mjs` (alias mappings)
