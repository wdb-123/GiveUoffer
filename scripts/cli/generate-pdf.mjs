 ...
 *
 * Usage:
 *   node Ucareer/generate-pdf.mjs <input.html> <output.pdf> [--format=letter|a4]
 *
...
// Ensure the workspace export boundary exists (fresh setup).
mkdirSync(resolve(projectRoot, 'workspace/ops/exports'), { recursive: true });
...
async function generatePDF() {
  ...
  // Resolve font paths relative to the resume template assets.
  const fontsDir = resolve(projectRoot, 'workspace/ops/templates/fonts');
  html = html.replace(
    /url\(['"]?\.\/fonts\//g,
    `url('file://${fontsDir}/`
  );
