#!/usr/bin/env node
/**
 * analyze-patterns.mjs — Rejection Pattern Detector for Ucareer
 *
 ...
 *
 * Run: npm run patterns --          (JSON to stdout)
 *      npm run patterns -- --summary (human-readable table)
 *      npm run patterns -- --min-threshold 3
 *      npm run patterns -- --self-test
 */
...
function analyze() {
  ...
  if (geoBlocker && geoBlocker.percentage >= 20) {
    recommendations.push({
      action: `Tighten location filters in workspace/profile/portals.yml -- ${geoBlocker.percentage}% of applications hit a geo-restriction blocker`,
