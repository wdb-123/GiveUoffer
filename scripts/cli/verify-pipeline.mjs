#!/usr/bin/env node
/**
 * verify-pipeline.mjs — Health check for Ucareer pipeline integrity
 *
 * Checks:
 ...
 * 6. No pending TSVs in workspace/ops/batch/tracker-additions/ (only in merged/ or archived/)
 * 7. states.yml canonical IDs for cross-system consistency
 *
 * Run: node Ucareer/verify-pipeline.mjs
 */
...
const APPS_FILE = existsSync(join(CAREER_OPS, 'workspace/ops/data/applications.md'))
  ? join(CAREER_OPS, 'workspace/ops/data/applications.md')
  : join(CAREER_OPS, 'applications.md');
const APPS_DIR = dirname(APPS_FILE);
const ADDITIONS_DIR = join(CAREER_OPS, 'workspace/ops/batch/tracker-additions');
const REPORTS_DIR = join(CAREER_OPS, 'workspace/jobs/reports');
const STATES_FILE = existsSync(join(CAREER_OPS, 'workspace/ops/templates/states.yml'))
  ? join(CAREER_OPS, 'workspace/ops/templates/states.yml')
  : join(CAREER_OPS, 'states.yml');

// Ensure required directories exist (fresh setup)
mkdirSync(join(CAREER_OPS, 'workspace/ops/data'), { recursive: true });
