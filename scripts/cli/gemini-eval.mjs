#!/usr/bin/env node
/**
 * gemini-eval.mjs — Gemini-powered Job Offer Evaluator for Ucareer
 *
 * A free-tier alternative to the Claude-based pipeline.
 * Reads evaluation logic from modes/oferta.md + modes/_shared.md,
 * reads the user's resume from workspace/profile/cv.md, and evaluates a Job Description
...
const PATHS = {
  ...
  profile:     join(ROOT, 'workspace', 'profile', '_profile.md'),
  profileYml:  join(ROOT, 'workspace', 'profile', 'profile.yml'),
  reports:     join(ROOT, 'workspace', 'jobs', 'reports'),
  tracker:     join(ROOT, 'workspace', 'ops', 'data', 'applications.md'),
};
...
if (args.length === 0 || args[0] === '--help' || args[0] === '-h') {
  console.log(`
╔══════════════════════════════════════════════════════════════════╗
║           Ucareer — Gemini Evaluator (free-tier)             ║
╚══════════════════════════════════════════════════════════════════╝
  ...
  OPTIONS
    ...
    --no-save        Do not save report to workspace/jobs/reports/ directory
    --help           Show this help
...
const profileContent = readFile(PATHS.profile,     'workspace/profile/_profile.md');
...
// ---------------------------------------------------------------------------
const systemPrompt = `You are Ucareer, an AI-powered job search assistant.
...
═══════════════════════════════════════════════════════
CANDIDATE RESUME (workspace/profile/cv.md)
═══════════════════════════════════════════════════════
${cvContent}
...
═══════════════════════════════════════════════════════
USER ARCHETYPES & NARRATIVE (workspace/profile/_profile.md)
═══════════════════════════════════════════════════════
${profileContent}
...
`;
    ...
    console.log(`\n✅  Report saved: workspace/jobs/reports/${filename}`);

    // Append tracker entry reminder
    console.log(`\n📊  Tracker entry (add to workspace/ops/data/applications.md):`);
    console.log(`    | ${num} | ${today} | ${company} | ${role} | ${score} | Evaluada | ❌ | [${num}](../../jobs/reports/${filename}) |`);
  } catch (err) {
