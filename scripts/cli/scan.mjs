 ...
 *
 * Scanner providers live in scripts/cli/providers/*.mjs and are loaded at startup. Each provider
 * exports a default object with:
 *   - id: string — matched against `provider:` in workspace/profile/portals.yml
 ...
 * scripts/cli/providers/. Local executable parsers use `scripts/cli/providers/local-parser.mjs` when
 * `parser.command` + `parser.script` are set in workspace/profile/portals.yml.
 *
 ...
 *   npm run scan --dry-run        # preview without writing files
 *   npm run scan --company Cohere # scan a single company
 *   npm run scan --verify         # Playwright-check each new URL; drop expired postings
 */
...
const PORTALS_PATH = process.env.CAREER_OPS_PORTALS || 'workspace/profile/portals.yml';
const SCAN_HISTORY_PATH = 'workspace/ops/data/scan-history.tsv';
const PIPELINE_PATH = 'workspace/ops/data/pipeline.md';
const APPLICATIONS_PATH = 'workspace/ops/data/applications.md';
const PROJECT_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../..');
const PROVIDERS_DIR = path.join(path.dirname(fileURLToPath(import.meta.url)), 'providers');

// Ensure required directories exist (fresh setup)
mkdirSync(path.join(PROJECT_ROOT, 'workspace/ops/data'), { recursive: true });
...
// ── Location filter ─────────────────────────────────────────────────
// Optional. If `location_filter` is absent from workspace/profile/portals.yml, all locations pass.
...
// Normalize a keyword list from workspace/profile/portals.yml: tolerates a bare string
...
async function main() {
  ...
  if (providers.size === 0) {
    console.error('Error: no scanner providers loaded from scripts/cli/providers/');
    process.exit(1);
  }

  // 2. Read workspace/profile/portals.yml
  if (!existsSync(PORTALS_PATH)) {
    console.error('Error: workspace/profile/portals.yml not found. Run onboarding first.');
    process.exit(1);
  }
  ...
  console.log(`\n→ Run /ucareer pipeline to evaluate new offers.`);
  console.log('→ Share results and get help: https://discord.gg/8pRpHETxa4');
}

// Only run main() when invoked directly (`npm run scan`), not when imported by tests.
