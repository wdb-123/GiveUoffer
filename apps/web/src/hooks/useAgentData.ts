...
type AgentDomainAction =
  | { type: "load"; providers: ProviderSummary[]; tasks: AgentTask[]; approvals: ApprovalRequest[] }
...
export function useAgentData(enabled = true) {
  ...
  async function refresh() {
    try {
      ...
      dispatch({ type: "load", providers, tasks, approvals });
  ...
  async function onCreateTask(promptOverride?: string, permissionMode?: CreateAgentTaskRequest["permissionMode"], attachments?: AgentAttachment[]) {
    ...
    const [tasks, approvals] = await Promise.all([getAgentTasks(), getApprovals()]);
    dispatch({ type: "set_tasks", tasks, approvals });
