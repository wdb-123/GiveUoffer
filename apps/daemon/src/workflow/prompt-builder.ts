import type { RouteDecision, SkillDefinition } from "@ucareer/shared";
...
export function buildAgentPrompt(input: {
  ...
  const firstConnectorTool = input.skill.connectorTools?.[0];
  const toolCallInstruction = firstConnectorTool
    ? [
      "- 需要读取后端工具时，只输出一行工具调用，不要编造工具结果。",
      `- 格式必须是：UC_TOOL_CALL {"tool":"${firstConnectorTool.id}","input":{...}}`,
    ...
    : [
      ...
      "- 如果用户请求需要工具能力但本轮没有暴露对应工具，请说明需要切换到合适工作流或请求进一步上下文。",
    ].join("\n");
...
处理原则：
1. 先验证后端路由是否正确；如果不正确，说明你改判成什么类型，并按更合适的职业工作流处理。
...
9. 不要直接提交任何外部申请；本地写入、删除、同步、安装、联网发送或破坏性命令必须先请求确认。
