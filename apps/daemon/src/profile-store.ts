...
import type { CareerProfileOverview } from "@offeru/shared";
...
export function createProfileStore(workspaceRoot: string): ProfileStore {
  const cvPath = join(workspaceRoot, "cv.md");
  const profilePath = join(workspaceRoot, "config/profile.yml");
  const profileOverlayPath = join(workspaceRoot, "modes/_profile.md");
