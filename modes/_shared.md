...
| File | Path | When |
|------|------|------|
| workspace/profile/cv.md | `workspace/profile/cv.md` | ALWAYS |
...
| workspace/profile/_profile.md | `workspace/profile/_profile.md` | ALWAYS (user archetypes, narrative, negotiation) |
| workspace/profile/writing-samples/ | `workspace/profile/writing-samples/` | When generating candidate-facing text — check `workspace/profile/_profile.md` for cached `## Writing Style` first; only scan files if absent |
...
**RULE: Read workspace/profile/_profile.md AFTER this file. User customizations in workspace/profile/_profile.md override defaults here.**
...
| North Star alignment | How well the role fits the user's target archetypes (from workspace/profile/_profile.md) |
...
After detecting archetype, read `workspace/profile/_profile.md` for the user's specific framing and proof points for that archetype.
...
1. Read workspace/profile/cv.md, workspace/profile/_profile.md, and workspace/profile/article-digest.md (if exists) before evaluating
1b. **First evaluation of each session:** Run `npm run sync-check`. If warnings, notify user.
2. Detect the role archetype and adapt framing per workspace/profile/_profile.md
...
9. **Tracker additions as TSV** -- NEVER edit applications.md directly. Write TSV in `workspace/ops/batch/tracker-additions/`.
...
**Check `workspace/profile/_profile.md` first.** If a `## Writing Style` section exists there, use it directly — do not re-scan the writing-samples files. Re-scanning is only needed when new samples are added or the user explicitly asks to recalibrate.
...
**If no cached style in `workspace/profile/_profile.md`:** Read all files in `workspace/profile/writing-samples/`, **skipping any file named `README.md`**. If no user-provided samples are found, skip style calibration and gently note — once, without pressure — that adding a writing sample (e.g. a past cover letter, a LinkedIn About section, any professional writing) would help tailor outputs to their voice. If samples exist, extract the markers below and write the result to `workspace/profile/_profile.md` under `## Writing Style` so future sessions skip this step.
...
After scanning (excluding any `README.md` files), write to `workspace/profile/_profile.md` only if at least one user-provided sample was found: find the existing `## Writing Style` section and replace the entire block up to the next `##` heading (or EOF) with the new content. If no `## Writing Style` section exists, append it. This ensures there is always exactly one canonical section. If no samples were found after filtering, do not write or modify the section.
