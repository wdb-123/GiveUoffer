...
Traite les URLs d'offres accumulees dans `workspace/ops/data/pipeline.md`. Le candidat ajoute des URLs quand il veut et lance ensuite `/ucareer pipeline` pour toutes les traiter d'un coup.

## Workflow

1. **Lire** `workspace/ops/data/pipeline.md` -> trouver les items `- [ ]` dans la section "En attente" / "Pending" / "Pendientes"
2. **Pour chaque URL en attente** :
   a. Calculer le prochain `REPORT_NUM` sequentiel (lire `workspace/jobs/reports/`, prendre le numero le plus eleve + 1)
...
- **Prefixe `local:`** : Lire le fichier local. Exemple : `local:workspace/jobs/jds/linkedin-pm-ai.md` -> lire `workspace/jobs/jds/linkedin-pm-ai.md`
...
1. Lister tous les fichiers dans `workspace/jobs/reports/`
