...
11. Generate the `.tex` file using `workspace/ops/templates/cv-template.tex`
12. Write to `workspace/ops/exports/cv-{candidate}-{company}-{YYYY-MM-DD}.tex`
13. Run: `npm run latex -- workspace/ops/exports/cv-{candidate}-{company}-{YYYY-MM-DD}.tex workspace/ops/exports/cv-{candidate}-{company}-{YYYY-MM-DD}.pdf`
...
The template at `workspace/ops/templates/cv-template.tex` uses `{{PLACEHOLDER}}` syntax:
...
| `{{EDUCATION}}` | LaTeX `\resumeSubheading` blocks from workspace/profile/cv.md Education section |
...
| `{{SKILLS}}` | LaTeX `\textbf{Category}{: items}` lines from workspace/profile/cv.md Technical Skills |
